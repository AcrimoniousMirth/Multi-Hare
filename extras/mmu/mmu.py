...
#######################
    ...
    def _ensure_safe_extruder_temperature(self, source="auto", wait=False):
        ...
        if new_target_temp > 0:
            self.gcode.run_script_from_command("SET_HEATER_TEMPERATURE HEATER=%s TARGET=%.1f" % (self.extruder_name, new_target_temp))
                                ...
                                try:
                                    ...
                                    if self.filament_pos != self.FILAMENT_POS_UNLOADED:
                                        if self.ttg_map[tool] == self.gate_selected:
                                            self.log_info("Target tool/gate is already selected, skipping unload on retry")
                                        ...
                                        elif new_sys_id != self.system_active and self.sensor_manager.check_all_sensors_in_path():
                                            self.log_info("Toolhead Swap: Skipping unload to gate, filament will remain mounted in %s" % self.mmu_toolhead.toolhead_name)
